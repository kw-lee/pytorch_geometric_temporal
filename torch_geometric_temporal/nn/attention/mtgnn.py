...
class DynamicMTGNNLayer(nn.Module):
    r"""An implementation of the dynamic graph version of MTGNN layer.
    ...
    def __init__(self, dilation_exponential: int, rf_size_i: int, kernel_size: int, j: int, residual_channels: int, 
                conv_channels: int, skip_channels: int, kernel_set: list, new_dilation: int, layer_norm_affline: bool,
                gcn_true: bool, seq_length: int, receptive_field: int, dropout: float, gcn_depth: int,
                propalpha: float):
        super(DynamicMTGNNLayer, self).__init__()
        self._dropout = dropout
        self._gcn_true = gcn_true
        ...
        if seq_length > receptive_field:
            self._normalization = nn.LayerNorm((residual_channels, seq_length - rf_size_j + 1),
                                                     elementwise_affine=layer_norm_affline)
                                                     
        else:
            self._normalization = nn.LayerNorm((residual_channels, receptive_field - rf_size_j + 1), 
                                                     elementwise_affine=layer_norm_affline)
    ...
    def forward(self, X: torch.FloatTensor, X_skip: torch.FloatTensor, A_tilde: Optional[torch.FloatTensor], 
                training: bool) -> torch.FloatTensor:
        """
        Making a forward pass of MTGNN layer.
...
class DynamicMTGNN(nn.Module):
    ...
    def __init__(self, gcn_true: bool, gcn_depth: int,
                 kernel_set: list, kernel_size: int, dropout: float,
                 dilation_exponential: int, conv_channels: int, residual_channels: int, 
                 skip_channels: int, end_channels: int, seq_length: int, in_dim: int, out_dim: int,
                 layers: int, propalpha: float, layer_norm_affline: bool):
        super(DynamicMTGNN, self).__init__()
        ...
        new_dilation = 1
        for j in range(1, layers+1):
            self._mtgnn_layers.append(DynamicMTGNNLayer(dilation_exponential=dilation_exponential,
                                                        rf_size_i=1,
                                                        kernel_size=kernel_size,
                                                        j=j, 
                                                        residual_channels=residual_channels,
    ...
    def forward(self, X_in: torch.FloatTensor,
                A_tilde: Optional[torch.FloatTensor]) -> torch.FloatTensor:
        """
        Making a forward pass of MTGNN.
